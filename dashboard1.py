...
def metrics():
    ...
    col1.metric(label ='Total Customer', value = df_selection.Gender.count(),delta
        ='ALL Customers')
...
def pie():
                 div1 = st.empty()
                 theme_plotly = None
                 fig = px.pie(df_selection,values='AnnualSalary', names='Department',
                 title='Customers by Countr')
...
def barchart():
                 div2 = st.empty()
                 theme_plotly = None
                 fig = px.bar(df_selection, y='AnnualSalary', x='Department', title='Controlled text sizes, positions and angles', text_auto='0.2s')
